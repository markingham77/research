...
SELECT
            ...
            f.event_timestamp,
            f.user_agent,
            CASE WHEN user_agent = 'mobile-api' THEN f.screenview_id ELSE f.pageview_id END AS view_id,
            page_start_index,
            ...
            ARRAY_CONSTRUCT_COMPACT(
                LOWER(query), filters_gender, filters_category, filters_product_type, filters_sale
            ) AS search_query,
        ...
        WHERE f.event_timestamp::DATE BETWEEN '2023-08-25' AND '2023-09-07'
        AND f.user_agent  IN ('web-backend-internal', 'mobile-api', 'unknown')
